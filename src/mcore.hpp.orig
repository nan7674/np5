...
namespace np5 { namespace mcore {
	...
	*/
	 ...
	 */
	inline double eval(std::valarray<double> const& poly, double const x) noexcept {
	...
	*/
	...
	P optimize_hj(F func, P p0, conf_opt const& cnf) {
		...
		for (size_t i = 0; i < cnf.num_iterations; ++i) {
			...
			while (true) {
				auto sr = neigbour_search_total(func, p1, step);
				if (sr.first) {
			...
			while (true) {
				...
				auto p = neigbour_search_total(func, p2, step);
